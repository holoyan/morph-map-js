...
export class MorphMapManager implements MorphInterface {
  private _map: MorphMapInterface = {}

  private static _instance?: MorphMapManager
  ...
  get(alias: string) {
    if (!(alias in this._map)) {
      throw new Error('morph map not found for alias: ' + alias)
    }
...
export const morphMap = MorphMapManager.create()
